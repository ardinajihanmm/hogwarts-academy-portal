...
try {
  ...
  $stmt = $db->prepare('INSERT INTO users (username,email,password,role,house,xp,level,created_at) VALUES (?,?,?,\'student\',?,0,\'Beginner Wizard\',NOW())');
  $stmt->execute([$username,$email,password_hash($password,PASSWORD_BCRYPT),$house]);
